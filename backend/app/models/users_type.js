...
Users.loginuser = (email,password,user_type, result) => {
    sql.query(`SELECT * FROM user WHERE email = ? AND password = ? AND user_type = ?`,
...
Users.alluser = (user_type, result) => {
  sql.query("SELECT * FROM user WHERE user_type = ?",[user_type], (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
...
Users.alluserdelete = (user_type, result) => {
  sql.query("DELETE FROM user WHERE user_type = ?",user_type, (err, res) => {
    if (err) {
      console.log("error: ", err);
      result(null, err);
      return;
